...
def init_plugin():
    ...
    config = {
        'plugin_name': 'M22CRCOOP',
        'plugin_type': 'sim'
    }
    return config

class M22CRCOOP(ConflictResolution):
    def __init__(self):
        super().__init__()
        ...
        self.cruise_spd = bs.traf.TrafficSpawner.spd
        
    
    def resolve(self, conf, ownship, intruder):
        """Velocity and altitude-only solving. Priority-aircraft goes faster.
        """
    ...
    def M22CR(self, conf, ownship, intruder, idx1, idx_pairs):
        ...
        prio = [False]* n_intr # Do we have priority in this conflict
            ...
            if los:
                ...
                if dist < self.dist_tol and intr_front_aligned:
                    ...
                    if intersection:
                        ...
                        # If there are positive ones, apply either the slowest or the fastest depending on priority
                        if pos_speeds:
                            if own_has_priority:
                                recd_speed[i] = max(pos_speeds)
                            else:
                                recd_speed[i] = min(pos_speeds)
                        elif neg_speeds:
                            if own_has_priority:
                                recd_speed[i] = min(neg_speeds)
                            else:
