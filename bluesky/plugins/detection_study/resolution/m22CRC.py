...
class M22CRC(ConflictResolution):
    def __init__(self):
        super().__init__()
        ...
        for i, idx_pair in enumerate(idx_pairs):
            ...
            if los:
                ...
                if dist < self.dist_tol and intr_front_aligned:
                    ...
                    recd_speed[i] = self.get_VO_speed(conf, ownship, intruder, idx1, idx2, own_has_priority)
    ...
    def get_VO_speed(self, conf, ownship, intruder, idx1, idx2, prio):
        ...
        if intersection:
            ...
            if pos_speeds:
                if prio:
                    return max(pos_speeds)
                else:
                    return min(pos_speeds)
            elif neg_speeds:
                if prio:
                    return min(neg_speeds)
                else:
                    return max(neg_speeds)
            else:
                # Do nothing I guess
