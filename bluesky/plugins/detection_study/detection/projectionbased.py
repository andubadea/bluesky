''' Projected-based conflict detection. '''
import numpy as np
...
class ProjectionBased(ConflictDetection):
    def __init__(self):
        super().__init__()
    ...
    def update(self, ownship, intruder):
        ...
        # Also do statebased detect for anything else
        confpairs_s, lospairs_s, inconf_s, tcpamax_s, qdr_s, \
            dist_s, dcpa_s, tcpa_s, tLOS_s, qdr_mat_s, dist_mat_s = \
                self.statebased_detect(ownship, intruder, self.rpz, self.hpz, self.dtlookahead)
        
        # for loop through these guys and check which conflicts are in open airspace
        if not self.add_sb_conflicts:
            # Only add open airspace conflicts, is this considered vanilla PROJECTEDBASED
            for idx_pair, pair in enumerate(confpairs_s):
                # Find the two idxs
                idx1, idx2 = bs.traf.id.index(pair[0]), bs.traf.id.index(pair[1])
                # Check if they're both in open airspace
                if bs.traf.actedge.edge_airspace_type[idx1] == 'open' and bs.traf.actedge.edge_airspace_type[idx2] == 'open':
                    # Add this conflict to the list of conflicts
                    self.confpairs.append(pair)
                    self.inconf[idx1] = inconf_s[idx1]
                    self.tcpamax[idx1] = tcpamax_s[idx1]
                    self.qdr.append(qdr_s[idx_pair])
                    self.dist.append(dist_s[idx_pair])
                    self.dcpa.append(dcpa_s[idx_pair])
                    self.tcpa.append(tcpa_s[idx_pair])
                    self.tLOS.append(tLOS_s[idx_pair])
                    self.projected_lats.append([bs.traf.lat[idx1], bs.traf.lat[idx2]])
                    self.projected_lons.append([bs.traf.lon[idx1], bs.traf.lon[idx2]])
                    self.conftrks.append([bs.traf.hdg[idx1], bs.traf.hdg[idx2]])
                    
        elif self.proj_filter:
            # We want to filter out the conflicts detected by statebased but not detected by projected based
            # in constrained airspace. Keep the ones that are open airspace
            idx_pairs_to_keep = []
            # Go through statebased pairs. If they're in open airspace, keep em. If they're in constrained, only
            # keep em if the pair is also in projected-based
            for idx_pair, pair in enumerate(confpairs_s):
                idx1, idx2 = bs.traf.id.index(pair[0]), bs.traf.id.index(pair[1])
                open_airspace = bs.traf.actedge.edge_airspace_type[idx1] == 'open' and bs.traf.actedge.edge_airspace_type[idx2] == 'open'
                
                if open_airspace or pair in self.confpairs:
                    idx_pairs_to_keep.append(idx_pair)
                    
            # Remake the values
            self.confpairs = [confpairs_s[i] for i in idx_pairs_to_keep]
            self.qdr = [qdr_s[i] for i in idx_pairs_to_keep]
            self.dist = [dist_s[i] for i in idx_pairs_to_keep]
            self.dcpa = [dcpa_s[i] for i in idx_pairs_to_keep]
            self.tcpa = [tcpa_s[i] for i in idx_pairs_to_keep]
            self.tLOS = [tLOS_s[i] for i in idx_pairs_to_keep]
            # Make the lat lons inconf and conftracks
            for pair in self.confpairs:
                idx1, idx2 = bs.traf.id.index(pair[0]), bs.traf.id.index(pair[1])
                self.inconf[idx1] = inconf_s[idx1]
                self.tcpamax[idx1] = tcpamax_s[idx1]
                self.projected_lats.append([bs.traf.lat[idx1], bs.traf.lat[idx2]])
            ...
            # Add all conflicts that were detected by statebased but not by projectedbased
            for idx_pair, pair in enumerate(confpairs_s):
                if pair not in self.confpairs:
                    # Find the two idxs
                    idx1, idx2 = bs.traf.id.index(pair[0]), bs.traf.id.index(pair[1])
                    # Add the info
                    self.confpairs.append(pair)
        ...
        self.lospairs = lospairs_s
        self.dist_mat = dist_mat_s
        self.qdr_mat = qdr_mat_s
    ...
    def get_projected_line_route(self, front_line, back_line, route_line, p1, current_loc, look_ahead_dist):
        ...
        if front_line.length < look_ahead_dist:
            ...
            if front_line.length == 0:
                ...
                look_ahead_line = LineString([p1, dummy_scaled.coords[-1]])
    ...
    def statebased_detect(self, ownship, intruder, rpz, hpz, dtlookahead):
        ''' Conflict detection between ownship (traf) and intruder (traf/adsb).'''
...
def plot_things(p_own, p_int, own_line, int_line, s_own, s_int, p_inter, lpr_own, lpr_int, pr_own, pr_int):
    ...
    p_int = gpd.GeoSeries([
                            p_int, # current point of intruder
                            ], crs='epsg:32633')
