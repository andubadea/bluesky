...
class RadarWidget(QGLWidget):
    ...
    def create_objects(self):
        ...
        # ------- Protected Zone -------------------------
        circlevertices = np.transpose(np.array((bs.settings.asas_pzr/2 * nm * np.cos(np.linspace(0.0, 2.0 * np.pi, VCOUNT_PZ)), bs.settings.asas_pzr/2 * nm * np.sin(np.linspace(0.0, 2.0 * np.pi, VCOUNT_PZ))), dtype=np.float32))
