...
class M2StateBasedAlloc(ConflictDetection):
    def __init__(self):
        super().__init__()
    ...
    def update(self, ownship, intruder):
        ...
        self.confpairs = self.filter_alloc()
    ...
    def filter_alloc(self): 
        # remove any confpairs of aircraft that are in different layer allocations
        confpairs_alloc = []
        for conf_pair in self.confpairs:
            acid_1, acid_2 = conf_pair
            
            # only remove conflicts if aircraft are in constrained airspace
            acidx_1 = bs.traf.id2idx(acid_1)
            acidx_2 = bs.traf.id2idx(acid_2)

            airspace_type1 = bs.traf.actedge.edge_airspace_type[acidx_1]
            airspace_type2 = bs.traf.actedge.edge_airspace_type[acidx_2]

            if airspace_type1 == 'constraned' and airspace_type2 == 'constrained':

                # now that they are in constrained only allow conflicts between aircraft in same angle-range
                alloc_1 = bs.traf.constrained_airspace_alloc[acidx_1]
                alloc_2 = bs.traf.constrained_airspace_alloc[acidx_2]

                if alloc_1 == alloc_2:
                    confpairs_alloc.append(conf_pair)

            else:
                # keep as confpairs
                confpairs_alloc.append(conf_pair)

        return confpairs_alloc
